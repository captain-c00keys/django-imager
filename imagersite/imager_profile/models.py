...
class ImagerProfile(models.Model):
    """The database model for the profile."""
    ...
    @classmethod
    def active(cls):
        """Pass the class to filter out is_active is True."""
        return cls.objects.filter(is_active=True)


@receiver(models.signals.post_save, sender=User)
def create_profile(sender, **kwargs):
    """Receiver that signals to create a profile."""
    if kwargs['created']:
